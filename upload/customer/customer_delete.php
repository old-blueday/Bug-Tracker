 ...
 *
 * Version:	$Id: customer_delete.php,v 1.8 2009/04/18 15:21:11 alex Exp $
 *
 */
...
$sql = "select customer_name from ".$GLOBALS['BR_customer_table']." where customer_id='".$_GET['customer_id']."'";
...
$delete_sql="delete from ".$GLOBALS['BR_customer_user_table']." where customer_id='".$_GET['customer_id']."'";
$GLOBALS['connection']->Execute($delete_sql) or DBError(__FILE__.":".__LINE__);

$delete_sql="delete from ".$GLOBALS['BR_customer_table']." where customer_id='".$_GET['customer_id']."'";
$GLOBALS['connection']->Execute($delete_sql) or DBError(__FILE__.":".__LINE__);
...
for ($i = 0; $i<sizeof($project_array); $i++) {
	$sql = "update proj".$project_array[$i]->getprojectid()."_report_table set
		reported_by_customer=null where reported_by_customer='".$_GET['customer_id']."'";
	...
	$sql = "update proj".$project_array[$i]->getprojectid()."_feedback_table set
		customer_id=null where customer_id='".$_GET['customer_id']."'";
	$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
}
