 ...
 *
 * Version:	$Id: login.php,v 1.12 2009/07/07 15:13:52 alex Exp $
 *
 */
...
$auth = false;
$sql="select * from ".$GLOBALS['BR_customer_user_table']." where email='".$_POST['email']."' and password='".md5($_POST['password'])."'  and account_disabled!='t'";
...
// ±b¸¹©M±K½X¿é¤J¿ù»~
	...
	$sql="select * from ".$GLOBALS['BR_customer_user_tmp_table']." where email='".$_POST['email']."' and password='".md5($_POST['password'])."'";
	$sql_result = $GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	...
	if ($sql_result->Recordcount() == 1) {
		...
		if ($sql_result->Recordcount() == 1) {
			...
			if ($account_disabled == 't') {
				...
				$sql = "update ".$GLOBALS['BR_customer_user_table']." set password='".md5($_POST['password'])."'
			...
			if ($FEEDBACK_SYSTEM['login_mode'] == "mode_customer") {
				...
				$sql = "insert into ".$GLOBALS['BR_customer_user_table']."(
					...
					'".$_POST['email']."', '".md5($_POST['password'])."', '', $now)";
				$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
...
if ($auth === true) {
	...
	$sql = "update ".$GLOBALS['BR_customer_user_table']." set last_login=$now 
			where customer_user_id='".$_SESSION[SESSION_PREFIX.'feedback_uid']."'";
