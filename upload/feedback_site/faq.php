 ...
 *
 * Version:	$Id: faq.php,v 1.14 2008/11/29 15:38:06 alex Exp $
 *
 */
...
$first_project = 0;
if ($_GET['project_id']) {
	$first_project = $_GET['project_id'];
}

if ($_GET['search_key'] != "") {
	$condition = " and (question ".PATTERN_KEYWORD." '%".$_GET['search_key']."%' or answer ".PATTERN_KEYWORD." '%".$_GET['search_key']."%')";
}
...
for ($i = 0; $i < sizeof($project_array); $i++) {
	...
	$count_sql = "select count(*) from ".$GLOBALS['BR_faq_content_table']." where project_id='".$this_id."' and is_verified='t' $condition";
	...
	if ($this_id == $_GET['project_id']) {
		echo "<option value=".$this_id." selected>".$project_array[$i]->getprojectname()." ($count_faq)</option>\n";
	} else {
		echo "<option value=".$this_id.">".$project_array[$i]->getprojectname()." ($count_faq)</option>\n";
	}
	if (!$first_project) {
		$first_project = $this_id;
	}
	if ($_GET['project_id'] == "") {
...
$get_class_sql = "select * from ".$GLOBALS['BR_faq_class_table']." where project_id='".$first_project."'";
...
$condition = "where project_id=".$first_project." and is_verified='t' ";
...
for ($i = 0; $i < sizeof($all_class_id); $i++) {
	if ($_GET['search_key'] != "") {
		$count_sql = "select count(*) from ".$GLOBALS['BR_faq_map_table']." where faq_class_id='".$all_class_id[$i]."' and
			faq_id in (select faq_id from ".$GLOBALS['BR_faq_content_table']." $condition)";
	} else {
		$count_sql = "select count(*) from ".$GLOBALS['BR_faq_map_table']." where faq_class_id='".$all_class_id[$i]."'";
	}
...
?>
	...
	<div id="sub_container" style="width:80%;">
...
if (($_GET['faq_class'] != "") && ($_GET['faq_class'] != -1)) {
	$condition .= " and (faq_id in (select faq_id from ".$GLOBALS['BR_faq_map_table']." where faq_class_id='".$_GET['faq_class']."'))";
}
	...
	echo '
			<tr>
				...
				<td align="left" class="'.$td_class.'">
					<a href="faq_show.php?faq_id='.$id.'&project_id='.$first_project.$extra_params.'&page='.$page.'">
						'.$question.'
					</a>
				</td>
