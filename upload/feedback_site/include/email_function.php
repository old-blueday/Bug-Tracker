 ...
 *
 * Version:	$Id: email_function.php,v 1.23 2010/07/27 09:24:17 alex Exp $
 *
 */
/*
...
function SendRemindPassowrd($email, $password, $register)
{
	...
	if ($register) {
		...
		$subject = "Your new password on ".$feedback_system_name."!!";
	}
	$subject = "=?UTF-8?B?".base64_encode($subject)."?=";
	
	$message .= '<p>This is you new password on '.$feedback_system_name.':</p>';

	$message .= '
		<p>account: '.$email.'<br>
		password: '.$password.'</p>';
	if ($register) {
		$message .= '<p>Please login in 10 days. Otherwise, the account would be disabled and you would have to register again.</p>';
	} else {
		$message .= '<p>If you did not request this new password, just login as you old password. The system
		will remove this request.</p>
		<p>Please login in 10 days. Otherwise, the new password would be disabled and you would have to request again.</p>';
	}
	$message .= '<p>Thanks,<br>
		'.$mail_from_name.'</p></body></html>';
...
function SendReportEmail($project_id, $report_id, $extra_cc = "")
{
	...
	$auto_email_sql = "select user_id from ".$GLOBALS['BR_proj_auto_mailto_table']." where project_id=".$project_id." and
			can_unsubscribe='f'";
	$auto_email_result = $GLOBALS['connection']->Execute($auto_email_sql) or DBError(__FILE__.":".__LINE__);
	while ($row = $auto_email_result->FetchRow()) {
		$user_id = $row["user_id"];
	...
	$disabled_sql = "select count(customer_user_id) from ".$GLOBALS['BR_customer_user_table']." where email='$created_by' and account_disabled!='t'";
	...
	while ($row = $auto_email_result->FetchRow()) {
		$email = $row["email"];
		$disabled_sql = "select count(customer_user_id) from ".$GLOBALS['BR_customer_user_table']." where email='$email' and account_disabled!='t'";
		$disabled_result = $GLOBALS['connection']->Execute($disabled_sql) or DBError(__FILE__.":".__LINE__);
		$count = $disabled_result->fields[0];
		if ($count == 0) {
			continue;
