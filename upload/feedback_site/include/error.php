 ...
 *
 * Version:	$Id: error.php,v 1.13 2010/07/26 09:05:25 alex Exp $
 *
 */
function MessageBoxPrint($error, $message, $ButtonInput)
{
	...
	if ($error) {
		$hint_image = '<img border="0" src="images/error.gif" width="70" height="70">';
		...
		$hint_image = '<img border="0" src="images/config.png" width="80" height="76">';
...
<div id="report_mesg_main">
	<div class="report_mesg_sub">
		<div class="mesg_left">
			<p align="center"><?php echo $hint_image?></p>
			<p><?php echo $hint?></p>
		</div>
...
function ErrorPrintOut($error_key, $arg_key = "", $arg_string = "")
{
	...
	if ($error_key == "timeout") {
		$ButtonInput = '<input class="button" type=button value="'.$STRING['back'].'" onclick="Redirect(\'index.php\');">';
	} else {
							   ...
							   $error_key, $arg_key = "", $arg_string = "")
{
	...
	$_SESSION[SESSION_PREFIX.'feedback_back_array'] = $back_array;
...
function WriteSyslog($log_type, $log_string_key, $arg_key = "", $arg_string = "")
{
	...
	$arg_string = addslashes($arg_string);

	$now = $GLOBALS['connection']->DBTimeStamp(time());
	$sql = "insert into ".$GLOBALS['BR_feedback_syslog_table']."(customer_email, ip, log_type,
			time, log_string_key, arg_key, arg_string) values('".$_SESSION[SESSION_PREFIX.'feedback_email']."',
			'".$_SERVER['REMOTE_ADDR']."', $log_type,
			$now, '$log_string_key', '$arg_key', '$arg_string')";

	$GLOBALS['connection']->Execute($sql) or die("Failed to do syslog.");
	$syslog_id = $GLOBALS['connection']->Insert_ID($GLOBALS['BR_feedback_syslog_table'], 'syslog_id');
	...
	if ($syslog_id % $flex == 0) {
		$count_sql = "SELECT count(syslog_id) FROM ".$GLOBALS['BR_feedback_syslog_table'];
		...
		if ($count > $SYSTEM['max_syslog']) {
			...
			$sql = "SELECT syslog_id FROM ".$GLOBALS['BR_feedback_syslog_table']." ORDER BY syslog_id ASC";
			$result = $GLOBALS['connection']->SelectLimit($sql, 1, $start);
			$remove_less_than = $result->fields[0];
			$sql = "DELETE from ".$GLOBALS['BR_feedback_syslog_table']." where syslog_id < ".$remove_less_than;
