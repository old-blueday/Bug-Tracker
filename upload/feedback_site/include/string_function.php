 ...
 *
 * Version:	$Id: string_function.php,v 1.2 2010/07/26 09:05:25 alex Exp $
 *
 */
function GetDefaultLang()
{
	...
	for ($i = 0; $i < sizeof($browser_lang); $i++) {
		...
		if (($lang == "zh-tw") || ($lang == "zh-sg") || ($lang == "zh-hk")) {
...
{
	if (!isset($_SESSION[SESSION_PREFIX.'feedback_uid']) || ($_SESSION[SESSION_PREFIX.'feedback_uid'] == "")) {
		$language = GetDefaultLang();
	} else {
		$user_id = $_SESSION[SESSION_PREFIX.'feedback_uid'];

		 $get_lang_sql = "select language from ".$GLOBALS['BR_customer_user_table']." 
			 where customer_user_id=".$user_id;
		 $get_lang_result = $GLOBALS['connection']->Execute($get_lang_sql) or DBError(__FILE__.":".__LINE__);
		 if ($get_lang_result->Recordcount() != 1) {
			 $language = GetDefaultLang();
		 } else {
...
function GetStringArray($language)
{
	$string_sql = "select * from ".$GLOBALS['BR_string_table']." where string_lang='".$language."'";
...
$STRING = GetStringArray($_SESSION[SESSION_PREFIX.'language']);
?>
