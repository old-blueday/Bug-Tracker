 ...
 *
 * Version:	$Id: status_delete.php,v 1.9 2009/04/18 16:24:56 alex Exp $
 *
 */
...
$sql = "select status_name from ".$GLOBALS['BR_status_table']." where status_id='".$_GET['status_id']."'";
...
$delete_sql="delete from ".$GLOBALS['BR_status_table']." where status_id='".$_GET['status_id']."'";
...
for ($i = 0; $i<sizeof($project_array); $i++) {
	$sql = "update proj".$project_array[$i]->getprojectid()."_report_table set
			status=null where status='".$_GET['status_id']."'";
	$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
}

$sql = "delete from ".$GLOBALS['BR_group_allow_status_table']." where status_id='".$_GET['status_id']."'";
$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
