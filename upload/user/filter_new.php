 ...
 *
 * Version:	$Id: filter_new.php,v 1.17 2008/11/30 03:46:29 alex Exp $
 *
 */
...
// 檢查是否非第一次連到本頁，如果非第一次，則處理使用者所輸入的 filter 資料
if ((isset($_POST['next_op']) ) || isset($_POST['action'])) {
	...
	// 當有輸入條件時,更新 filter 的資料，並判斷是否為第一個 filter list
	// 來決定是否要以 operator 連接
	if ($this_filter != "") {
	...
	// 由於 PHP 會自動幫 ' 加上 \ 成 \'
	// 所以我們以 stripslashes()來去除 filter_name 字串中的所有斜線
...
if ($_SESSION[SESSION_PREFIX.'new_filter']['text_condition'] != "") {
	...
	<div id="sub_container" style="width:500;">
...
// 按下 donew 了,所以執行新增的動作
if ($_POST['action']) {
	// 先計算使用的的 filter 是否在 $MAX_FILTER_NUM 個以內，最多只能允許 $MAX_FILTER_NUM 個
	...
	$real_condition = addslashes($_SESSION[SESSION_PREFIX.'new_filter']['real_condition']);
	$text_condition = addslashes($_SESSION[SESSION_PREFIX.'new_filter']['text_condition']);
	if (!$_SESSION[SESSION_PREFIX.'new_filter']['real_condition']) {
	...
	$filter_sql = "insert into ".$GLOBALS['BR_filter_table']."(user_id, filter_name, real_condition, text_condition, share_filter) 
			values('".$_SESSION[SESSION_PREFIX.'uid']."', '".$_POST['filter_name']."',
			'".$real_condition."', '".$text_condition."', '".$_POST['share_filter']."')";
