 ...
 *
 * Version:	$Id: user_setting.php,v 1.12 2009/05/03 09:19:43 alex Exp $
 *
 */
...
// 從 userfilter 的 table 中，找出使用者關於 filter 的設定有哪些
$filter_sql = "select * from ".$GLOBALS['BR_filter_table']." where user_id='".$_GET['user_id']."'";
$filter_result = $GLOBALS['connection']->Execute($filter_sql) or DBError(__FILE__.":".__LINE__);
$line = $filter_result->Recordcount();
// 如果有使用者 filter 的資料，則顯示。否則顯示 None
if ($line != 0) {
	while ($row = $filter_result->FetchRow()){
		$filter_id = $row["filter_id"];
		$filter_name = $row["filter_name"];
