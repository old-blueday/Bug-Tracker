 ...
 *
 * Version:	$Id: user_donew.php,v 1.10 2010/07/27 09:24:17 alex Exp $
 *
 */
...
$check_user_sql = "select * from ".$GLOBALS['BR_user_table']." where username='".$_POST['username']."'";
...
$adduser_sql = "insert into ".$GLOBALS['BR_user_table']."(username, group_id, email, realname, created_date, password, language)
				values('".$_POST['username']."', '".$_POST['group_id']."', 
				'".$_POST['email']."', '".$_POST['realname']."', $now,'".$md5_passwd."', '".$_POST['language']."')";
...
for ($i=0; $i<$count_project; $i++) {
	...
	if (isset($_POST[$project_id])){
		$user_list_sql="insert into ".$GLOBALS['BR_proj_access_table']."(user_id, project_id, last_read) 
			values('".$user_id."', '".$_POST[$project_id]."', '1999-01-01')";
