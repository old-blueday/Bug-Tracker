 ...
 *
 * Version:	$Id: user_edit.php,v 1.13 2008/11/30 03:46:29 alex Exp $
 *
 */
...
$user_sql = "select * from ".$GLOBALS['BR_user_table']." where user_id='".$_GET['user_id']."'";
...
?>
	...
	<div id="sub_container" style="width:600;">
...
if ($_GET['user_id'] == $_SESSION[SESSION_PREFIX.'uid']) {
	$group_sql = "select group_name from ".$GLOBALS['BR_group_table']." where group_id='".$_SESSION[SESSION_PREFIX.'gid']."'";
	...
	while ($all_group_row = $group_result->FetchRow()) {
		...
		$the_group_name=$all_group_row["group_name"];
...
<?php
// 如果是有權修改一般使用者，則顯示選取使用者可以讀取的程式的表單
...
<?php
	...
	for ($i=0; $i < sizeof($project_array); $i++) {
		...
		$check_list = "select * from ".$GLOBALS['BR_proj_access_table']." where user_id='".$_GET['user_id']."' and project_id='$project_id'";
