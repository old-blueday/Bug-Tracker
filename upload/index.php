 ...
 *
 * Version:	$Id: index.php,v 1.23 2008/11/30 03:46:28 alex Exp $
 *
 */
...
<?php
	...
	while ($row = $sql_result->FetchRow()){
		...
		$visible_sql = "select * from ".$GLOBALS['BR_proj_access_table']." where user_id='".$_SESSION[SESSION_PREFIX.'uid']."' and project_id='$project_id'";
		$visible_result = $GLOBALS['connection']->Execute($visible_sql) or DBError(__FILE__.":".__LINE__);
		$line = $visible_result->Recordcount();
      
		// 如果該程式的允許清單中有該使用者，或使用為 admin,則顯示該程式的討論區
		if (($line != 0) || ($_SESSION[SESSION_PREFIX.'uid'] == 0)) {
			...
			<tr>
				...
				<td class="line'.($count%2).'">
					<a href="report/project_list.php?project_id='.$project_id.'"><b>'.$project_name.'</b></a><br>';
			...
			if ($_SESSION[SESSION_PREFIX.'uid'] == 0) {
				...
				$subscribe_sql = "select can_unsubscribe from ".$GLOBALS['BR_proj_auto_mailto_table'].
					" where project_id=".$project_id." and user_id=".$_SESSION[SESSION_PREFIX.'uid'];
