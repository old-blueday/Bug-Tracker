 ...
 *
 * Version:	$Id: report_function.php,v 1.36 2010/03/18 15:13:56 alex Exp $
 *
 */
/* 
...
function GetReportOutput($project_id, $report_id, $action)
{
	...
	$log_sql = "select log_id,user_id,post_time,filename,description from proj".$project_id."_report_log_table 
			where report_id='".$report_id."' order by log_id ASC";
	...
	if ($action == "update") {
		...
		for ($i=0; $i<sizeof($status_array); $i++) {
			...
			$status_name = $status_array[$i]->getstatusname();
		...
		if ($statusclass) {
			$return_string .= $statusclass->getstatusname();
		}
	}
	...
	if ($action == "update") {
		...
		$all_area = "select * from ".$GLOBALS['BR_proj_area_table']." where project_id='".$project_id."' and area_parent=0 order by area_name";
		...
		if ($line == 0) {
			...
			$return_string .= "<select size=\"1\" name=\"area\" onChange=\"change()\"><option></option>";
			$selected_area="";
			...
			$return_string .= "</select>/<select size=\"1\" name=\"minor_area\" onChange=\"area_assign()\">";
           
			if ($selected_area != 0) {
				$all_minor_area = "select * from ".$GLOBALS['BR_proj_area_table']." where project_id='".$project_id."' and area_parent='$selected_area' order by area_name";
