 ...
 *
 * Version:	$Id: email_function.php,v 1.29 2010/07/27 09:24:17 alex Exp $
 *
 */
/*
...
function SendUpdateUserEamil($username, $password, $type)
{
	...
	$email_sql = "select email from ".$GLOBALS['BR_user_table']." 
		where username='".$username."' and account_disabled!='t'";
	$email_result = $GLOBALS['connection']->Execute($email_sql) or DBError(__FILE__.":".__LINE__);
	if ($email_result->Recordcount() != 1) {
		return -1;
	}
	$email = $email_result->fields["email"];
	...
	if ($type == "password") {
		...
		$message .= '<p>Your password on '.$SYSTEM['program_name'].' has been changed.</p>';
	} else {
		$subject = "Welcome to ".$SYSTEM['program_name']."!!";
		$message .= '<p>Your account on '.$SYSTEM['program_name'].' has been created.</p>';
	}
	...
	$message .= '
		...
		<p>Please login here:<br>
		<a href="http://'.$_SERVER['HTTP_HOST'].$GLOBALS["SYS_URL_ROOT"].'/index.php">
			http://'.$_SERVER['HTTP_HOST'].$GLOBALS["SYS_URL_ROOT"].'/index.php</a></p>

		<p>Thanks,<br>
		'.$SYSTEM['mail_from_name'].'</p></body></html>';
...
function SendReportEmail($project_id, $report_id, $extra_to_uid = "")
{
	...
	if ($area != "") {
		$area_sql = "select area_id, owner from ".$GLOBALS['BR_proj_area_table']." where project_id='".$project_id."' and area_name='".$area."' and area_parent=0";
		...
		if ($line == 1) {
			...
			if (($minor_area != "") && ($area_id != "")) {
				$minor_area_sql = "select owner from ".$GLOBALS['BR_proj_area_table']." 
					where project_id='".$project_id."' and area_name='".$minor_area."' and area_parent=".$area_id;
...
function SendFAQEmail($faq_id, $extra_cc_uid)
{
	...
	$subject = "[".$project_name."][FAQ][id:".$faq_id."] Please verify this FAQ";
	...
	$message = '<html><body><style type="text/css">
        ...
        //  --></style>';
	$message .= '<p>Hi,</p>
		<p>Please help me to verify this FAQ and update the FAQ status on the system.</p>
		<p>--</p>
		<p>Question:</p>
		<blockquote>'.$question.'
		</blockquote>
