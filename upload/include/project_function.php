 ...
 *
 * Version:	$Id: project_function.php,v 1.17 2010/07/27 09:24:17 alex Exp $
 *
 */
class projectclass {
	var $project_id;
...
function ConditionByFilterSearch($chosen_filter, $label, $search_key, $search_type=SEARCH_TYPE_CONTENT)
{
	...
	if ($chosen_filter != 0) {
		...
		$get_filter_sql = "select * from ".$GLOBALS['BR_filter_table']." where filter_id='$chosen_filter'";
	...
	if ($search_key != "") {
		if ($search_type == SEARCH_TYPE_AREA) {
			$area_match="(area ".PATTERN_KEYWORD." '%".$search_key."%')";
			$minor_area_match="(minor_area ".PATTERN_KEYWORD." '%".$search_key."%')";
	...
	if ($label) {
		$label_condition = "report_id in (SELECT report_id FROM ".$GLOBALS['BR_label_mapping_table']." WHERE label_id='$label')";
		if ($condition == "") {
