 ...
 *
 * Version:	$Id: faq_doedit.php,v 1.11 2010/07/26 09:05:25 alex Exp $
 *
 */
...
// Get FAQ Content
$sql = "select * from ".$GLOBALS['BR_faq_content_table']."  where faq_id='".$_POST['faq_id']."'";
...
// Get project data
$project_sql = "select * from ".$GLOBALS['BR_project_table']." where project_id='".$project_id."'";
...
if ($_POST['is_verified'] != 't') {
	...
	$sql = "update ".$GLOBALS['BR_faq_content_table']." set question='".$_POST['question']."',
			answer='".$_POST['answer']."', last_update=$now, is_verified='f', verified_by=null,
			verified_date=null, assign_to='".$_POST['assign_to']."' 
	...
	if ($org_is_verified != 't') {
		$sql = "update ".$GLOBALS['BR_faq_content_table']." set 
			...
			last_update=$now, is_verified='t', verified_by='".$_SESSION[SESSION_PREFIX.'uid']."',
		...
		$sql = "update ".$GLOBALS['BR_faq_content_table']." set 
			question='".$_POST['question']."', answer='".$_POST['answer']."', 
			last_update=$now, 
			assign_to='-1' where faq_id='".$_POST['faq_id']."'";
	}
}
...
// Remove old category
$sql = "delete from ".$GLOBALS['BR_faq_map_table']." where faq_id='".$_POST['faq_id']."'";
$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
...
for ($i=0; $i < sizeof($all_belong_class); $i++) {
	...
	$sql = "insert into ".$GLOBALS['BR_faq_map_table']."(faq_id, faq_class_id) 
			values('".$_POST['faq_id']."', '".$all_belong_class[$i]."')";
	$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
}
