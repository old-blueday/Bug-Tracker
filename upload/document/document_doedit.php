 ...
 *
 * Version:	$Id: document_doedit.php,v 1.11 2010/07/26 09:05:25 alex Exp $
 *
 */
...
if (($filename == "") && ($_POST['delete_old_file'] == "Y")){
	...
	$sql = "update ".$GLOBALS['BR_document_table']." set
			subject='".$_POST['subject']."', group_class='".$_POST['group_class']."',
			allow_other_group='".$_POST['allow_other_group']."',
			description='".$_POST['description']."',
			created_by='".$_SESSION[SESSION_PREFIX.'uid']."',
			last_update=$now, filename='', filedata='' 
			where document_id='".$_POST['document_id']."'";
} elseif ($filename == "") {
	if ($GLOBALS['SYS_FILE_IN_DB'] != 1) {
		if ($old_filename != "") {
	...
	$sql = "update ".$GLOBALS['BR_document_table']." set
			...
			description='".$_POST['description']."',
			created_by='".$_SESSION[SESSION_PREFIX.'uid']."',
			last_update=$now, filename='".$filename."', filedata='".$filedata."' 
			where document_id='".$_POST['document_id']."'";
}
...
$history_sql = "INSERT INTO ".$GLOBALS['BR_document_history_table']."(
                ...
                FROM ".$GLOBALS['BR_document_table']." WHERE document_id='".$_POST['document_id']."'";
...
if (($GLOBALS['SYS_FILE_IN_DB'] != 1) && isset($new_old_filename) && $new_old_filename != $old_filename) {
	$sql = "UPDATE ".$GLOBALS['BR_document_history_table']." SET filename='".$new_old_filename."' WHERE
			document_history_id=".$document_history_id;
	$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
}

// Remove old category
$sql = "delete from ".$GLOBALS['BR_document_map_table']." where document_id='".$_POST['document_id']."'";
$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
...
for ($i=0; $i < sizeof($all_belong_class); $i++) {
	...
	$sql = "insert into ".$GLOBALS['BR_document_map_table']."(document_id, document_class_id) 
			values('".$_POST['document_id']."', '".$all_belong_class[$i]."')";
	$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
}
