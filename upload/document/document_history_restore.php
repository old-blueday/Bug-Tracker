 ...
 *
 * Version:	$Id: document_history_restore.php,v 1.4 2009/04/18 15:32:39 alex Exp $
 *
 */
...
$sql = "SELECT document_id,filename FROM ".$GLOBALS['BR_document_history_table']."
		WHERE document_history_id='".$_GET['document_history_id']."'";
...
$restore_sql = "UPDATE ".$GLOBALS['BR_document_table']." SET
				subject=(SELECT subject FROM ".$GLOBALS['BR_document_history_table']." WHERE document_history_id='".$_GET['document_history_id']."'),
				created_by=(SELECT created_by FROM ".$GLOBALS['BR_document_history_table']." WHERE document_history_id='".$_GET['document_history_id']."'),
				last_update=$now,
				description=(SELECT description FROM ".$GLOBALS['BR_document_history_table']." WHERE document_history_id='".$_GET['document_history_id']."'),
				filename=(SELECT filename FROM ".$GLOBALS['BR_document_history_table']." WHERE document_history_id='".$_GET['document_history_id']."'),
				filedata=(SELECT filedata FROM ".$GLOBALS['BR_document_history_table']." WHERE document_history_id='".$_GET['document_history_id']."')
                WHERE document_id=".$document_id;
...
if ($GLOBALS['SYS_FILE_IN_DB'] == 0) {
	if (isset($new_old_filename)) {
		$sql = "UPDATE ".$GLOBALS['BR_document_history_table']." SET filename='".$new_old_filename."' WHERE document_history_id='".$new_document_history_id."'";
		$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	}
	if (isset($new_history_filename)) {
		$sql = "UPDATE ".$GLOBALS['BR_document_table']." SET filename='".$new_history_filename."' WHERE document_id=".$document_id;
		$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	}
}
