 ...
 *
 * Version:	$Id: feedback_report_import.php,v 1.29 2008/12/09 02:32:55 alex Exp $
 *
 */
...
// 取得允許使用者使用的 status 名稱
$group_status_sql = "select * from ".$GLOBALS['BR_group_allow_status_table']." where group_id='".$_SESSION[SESSION_PREFIX.'gid']."'";
...
$log_sql = "select * from proj".$_GET['project_id']."_feedback_content_table 
			where report_id=".$_GET['report_id']." order by content_id ASC";
...
<div id="current_location">
	...
	<a href="feedback_list.php?project_id=<?php echo $_GET['project_id'].$extra_params?>"><?php echo $project_name." ".$STRING['title_feedback']?></a> /
...
?>
			...
			<td class="content">
<?php
	$all_area_sql = "select * from ".$GLOBALS['BR_proj_area_table']." where project_id='".$_GET['project_id']."' and area_parent=0 order by area_name";
	...
	// 如果有設定 Area 及 Minor Area 的話，就列出下拉式選單以供選取
	// 如果沒有則輸出文字方塊以供輸入
	if($root_area_line!=0){
		echo "<select size=\"1\" name=\"area\" onChange=\"change()\"><option></option>";
		while ($root_area_row = $root_area_result->FetchRow()){
			$area_name = $root_area_row["area_name"];
			echo "<option>$area_name</option>";
		}
		echo "</select>/<select size=\"1\" name=\"minor_area\" onChange=\"area_assign()\"></select>";
	}else{
		echo '
