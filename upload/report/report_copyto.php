 ...
 *
 * Version:	$Id: report_copyto.php,v 1.7 2009/04/18 16:16:08 alex Exp $
 *
 */
...
$project_sql = "select project_name from ".$GLOBALS['BR_project_table']." where project_id='".$_POST['project_id']."'";
...
// 取得原始資料
$get_report_sql = "select * from proj".$_POST['project_id']."_report_table where report_id='".$_POST['report_id']."'";
...
$log = "<font color=red>Copy From <b>$project_name</b> ID:<b>".$_POST['report_id']."</b></font>";
$log = addslashes($log);
...
$insert_sql = "insert into proj".$_POST['copyto_project_id']."_report_log_table( 
			...
			$copyto_report_id, ".$_SESSION[SESSION_PREFIX.'uid'].", $now, '$log')";
$GLOBALS['connection']->Execute($insert_sql) or DBError(__FILE__.":".__LINE__);
...
while ($row = $all_log_result->FetchRow()) {
	...
	$row['filename'] = addslashes($row['filename']);
	
	$insert_sql = "insert into proj".$_POST['copyto_project_id']."_report_log_table( 
			report_id, user_id, post_time, description, filename, filedata) 
			values($copyto_report_id, ".$row['user_id'].", 
			'".$row['post_time']."', '".$row['description']."', 
			'".$row['filename']."', (select filedata from 
...
$new_seealso_sql = "insert into proj".$_POST['copyto_project_id']."_seealso_table(
					...
					'".$copyto_report_id."','".$_POST['project_id']."',
					'".$_POST['report_id']."')";
...
$new_seealso_sql = "insert into proj".$_POST['project_id']."_seealso_table(
					report_id, see_also_project, see_also_id) values(
					'".$_POST['report_id']."','".$_POST['copyto_project_id']."',
					'".$copyto_report_id."')";
$GLOBALS['connection']->Execute($new_seealso_sql) or DBError(__FILE__.":".__LINE__);
...
if ($GLOBALS['SYS_FILE_IN_DB'] != 1) {
	...
	for ($i = 0; $i<sizeof($key_array); $i++) {
		...
		$update_sql = "update proj".$_POST['copyto_project_id']."_report_log_table set
					filename='".$filename."' where log_id=".$key_array[$i];
