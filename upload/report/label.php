 ...
 *
 * Version:	$Id: label.php,v 1.4 2009/07/07 15:13:52 alex Exp $
 *
 */
session_start();
...
case 'new':
	...
	if (utf8_strlen(stripslashes($_POST['label_name'])) > 30) {
	...
	$sql = "SELECT label_id FROM ".$GLOBALS['BR_label_table']." WHERE label_name='".$_POST['label_name']."' and project_id='".$_POST['project_id']."'";
	...
	$sql = "INSERT INTO ".$GLOBALS['BR_label_table']."(project_id, label_name, color)
			VALUES('".$_POST['project_id']."', '".$_POST['label_name']."', ".$color.")";
...
case 'apply':
	...
	for ($i = 0; $i < sizeof($bugids); $i++) {
		$sql = "DELETE FROM ".$GLOBALS['BR_label_mapping_table']."
		    WHERE report_id='".$bugids[$i]."' and label_id='".$_POST['label_id']."'";
		$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
		$sql = "INSERT INTO ".$GLOBALS['BR_label_mapping_table']."(report_id, label_id)
		    VALUES('".$bugids[$i]."', '".$_POST['label_id']."')";
		$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	}
...
case 'remove':
	...
	for ($i = 0; $i < sizeof($bugids); $i++) {
		$sql = "DELETE FROM ".$GLOBALS['BR_label_mapping_table']."
		    WHERE label_id='".$_POST['label_id']."' and report_id='".$bugids[$i]."'";
		$GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	}
