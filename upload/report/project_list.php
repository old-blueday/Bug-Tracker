 ...
 *
 * Version:	$Id: project_list.php,v 1.43 2010/07/27 09:24:17 alex Exp $
 *
 */
...
function GetUserFilter($uid)
{
	$filters = array();
	// User's filter
	$sql = "select * from ".$GLOBALS['BR_filter_table']." where user_id='".$uid."' order by filter_name";
	$result = $GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	$line = $result->Recordcount();
	if ($line == 0) {
		return $filters;
...
function GetSharedFilter($userarray, $uid)
{
	...
	$sql = "select * from ".$GLOBALS['BR_filter_table']." 
		where share_filter='t' and user_id!='".$uid."' order by user_id,filter_name";
	$result = $GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	$line = $result->Recordcount();
	if ($line == 0) {
		return $filters;
...
{
	$labels = array();
	$sql = "select * from ".$GLOBALS['BR_label_table']." where project_id=".$project_id;
	$result = $GLOBALS['connection']->Execute($sql) or DBError(__FILE__.":".__LINE__);
	$line = $result->Recordcount();
	if ($line == 0) {
		return $labels;
...
if ($_SESSION[SESSION_PREFIX.'uid'] != 0) {
	...
	$check_auth_sql = "select * from ".$GLOBALS['BR_proj_access_table']." 
		where user_id='".$_SESSION[SESSION_PREFIX.'uid']."' and project_id='".$_GET['project_id']."'";
...
if ( isset($_GET['search_key']) && (preg_match("/^#+[0-9]+[0-9]*$/", $_GET['search_key']))) {
	...
	$get_report_sql = "select * from proj".$_GET['project_id']."_report_table 
			where report_id='$search_report_id'";
...
<div id="current_location" class="project_list">
	...
	/<a href="../index.php"><?php echo $STRING['title_project_list']?></a>/<?php echo $project_name?>
</div>
	...
	<table width="100%" border="0">
		<tr>
			<td  align="left" nowrap>
				...
				<a href="project_list.php?project_id=<?php echo $_GET['project_id']?>"><tt class="outline"><?php echo $project_name?></tt></a>
			</td>
...
?>
	...
	<div id="sub_container" style="width:95%;">
...
for ($i = 0; $i < sizeof($show_column_array); $i++) {
	...
	if ($setting_row[$show_column] == 't') {
		...
		case "status";
			...
			break;
		case "area":
			$all_area = "select * from ".$GLOBALS['BR_proj_area_table']." where project_id='".$_GET['project_id']."' and area_parent=0 order by area_name";
		...
		case "minor_area":
			if (isset($_GET['filter_area']) && $_GET['filter_area'] != -1) {
				$all_minor_area = "select * from ".$GLOBALS['BR_proj_area_table']." where project_id='".$_GET['project_id']."' and area_parent='".$_GET['filter_area']."' order by area_name";
